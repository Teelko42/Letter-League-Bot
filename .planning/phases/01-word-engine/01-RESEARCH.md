...
| WENG-04 | Score words using Wild mode rules (multipliers permanently assigned to letters) | Core Wild mode behavior confirmed; stacking edge case flagged as open question requiring live-game verification |
...
Two Wild mode edge cases could not be definitively resolved from public documentation: (1) whether two bonded word-multiplier tiles in the same word multiply multiplicatively (x4 for two DW tiles) or additively (x2+x2=x4 are equivalent but TW+DW = x5 vs x6 differs), and (2) whether an original multiplier square becomes a normal square after a tile bonds with it (i.e., can a second tile placed on the same square inherit that multiplier). These require live game testing to confirm. The plan must treat them as flagged open questions with a test-driven verification spike.
...
- **Generating blank tile permutations upfront:** Do not pre-expand all possible blank tile substitutions. Instead, branch at the point of placement in the GADDAG traversal — only explore the 26 branches that the GADDAG actually has arcs for at that node.
...
**What goes wrong:** GADDAG is ~5x larger than a DAWG. A 170k-word list that takes 2MB as a trie can reach 10-15MB as a full GADDAG. Python dict overhead amplifies this.
**Why it happens:** Every word with N letters generates N paths through the graph. Python dict per node adds ~200-400 bytes overhead.
**How to avoid:** Do NOT store a full `dict` object per node. Instead, use a compact representation: a top-level `dict[str, int]` where values are indices into a flat node array. This is the "flat GADDAG" approach used in production Scrabble engines.
...
Confirmed from official Discord support documentation and TV Tropes wiki:
...
**Confirmed:** Wild mode makes extending existing high-multiplier words extremely powerful because all preexisting tiles keep their bonded multipliers. Adding a single letter to a word with two DW-bonded tiles earns x2 x2 = x4 on all existing letters.
...
1. **Wild mode: Do bonded word multipliers stack multiplicatively?**
   - What we know: Wild mode permanently bonds multipliers to tiles. When two DW-bonded tiles appear in the same word, the game likely multiplies x2 x2 = x4 (same as standard Scrabble with two DW squares). This is the only internally consistent behavior.
   - What's unclear: The official FAQ and TV Tropes wiki confirm permanent bonding but do not specify the stacking formula. A TW-bonded tile + DW-bonded tile in the same word is x3 x2 = x6 under multiplicative stacking, x5 under additive.
   - Recommendation: Implement multiplicative stacking (consistent with standard Scrabble multi-multiplier rules). Add a test case that verifies against a live game screenshot. Mark as validation-required in Phase 1 plan.

2. **Wild mode: Does the original square remain active after tile bonding?**
   - What we know: The rule states the multiplier "permanently bonds to the tile." It does not say whether the square itself remains a multiplier square.
   - What's unclear: If a second tile is placed adjacent and uses the same square (edge case on board expansion), does that second tile also bond the multiplier? Or is the square now a normal square?
   - Recommendation: Implement as "square remains active" (conservative, higher-scoring interpretation). Validate with live game testing in Phase 1 verification step. If wrong, the fix is one-line (set `square_multiplier = NONE` after bonding).
...
3. **Multiplier layout: exact (row, col) positions on 27x19 board**
   ...
   - What's unclear: All exact positions.
   - Recommendation: Extract from screenshot as part of Wave 0 / Plan 1 setup. Store as a constant dict in `board.py`. The engine architecture is designed to accept this as configuration, so the extraction does not block algorithmic work.
...
- [Wikitia - Letter League](https://wikitia.com/wiki/Letter_League) — Confirmed 4 multiplier types (2L/3L/2W/3W), standard Scrabble tile values, blank tiles
...
- [Medium: "Coding The World's Fastest Scrabble Program in Python"](https://medium.com/@aydinschwa/coding-the-worlds-fastest-scrabble-program-in-python-2aa09db670e3) — Anchor squares and cross-check definitions; step-by-step Python move generation walkthrough

### Tertiary (LOW confidence — requires live-game validation)
- Wild mode word multiplier stacking formula (multiplicative vs additive) — inferred from standard Scrabble rules; not confirmed by official Letter League documentation
...
- Letter League Wild mode (core): MEDIUM — permanent bonding confirmed; "individual letters keep multiplier" confirmed
- Letter League Wild mode (stacking edge cases): LOW — multiplicative stacking assumed from Scrabble convention; not officially confirmed
