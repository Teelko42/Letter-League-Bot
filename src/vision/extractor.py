...
EXTRACTION_PROMPT = (
    ...
    "  - The center of the board (marked with a green dot) is at row 9, column 13.\n"
    "\n"
    "COORDINATE LABELS AND GRIDLINES ON THE IMAGE:\n"
    "  Red numbers and gridlines have been overlaid on the image:\n"
    "  - COLUMN NUMBERS (0-26) are printed along the TOP edge of the board.\n"
    "  - ROW NUMBERS (0-18) are printed along the LEFT edge of the board.\n"
    "  - THIN RED GRIDLINES mark every cell boundary.\n"
    "  - A GREEN DOT marks the center star at (9,13).\n"
    "  Use the gridlines to determine which cell each tile belongs to.\n"
    "  For each tile, find the grid cell it sits in, then read the column number\n"
    "  above that cell and the row number to the left of that cell.\n"
    "\n"
    "MULTIPLIER SQUARES:\n"
    ...
    "  - Tiles freshly placed last turn have a GREEN OUTLINE/HIGHLIGHT around\n"
    "    them. This green is the recently-played indicator, NOT a DW square.\n"
    "    For a tile with a green outline, report the multiplier as NONE unless\n"
    "    you can clearly see the underlying square's colour through/around it.\n"
    "\n"
    "EXTRACT:\n"
    "\n"
    "1. BOARD CELLS — Only cells that contain a placed tile (skip empty squares).\n"
    "   For each placed tile, record:\n"
    "     - row: 0-indexed row number — read from the row labels on the left edge\n"
    "     - col: 0-indexed column number — read from the column labels on the top edge\n"
